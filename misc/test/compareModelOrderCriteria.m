...
hold on
plot(x,x*log(N)/2,'LineWidth',2,'DisplayName',['BIC, N=' num2str(N)])
plot(x,x*log(.1*N)/2,'LineWidth',2,'DisplayName',['BIC, N=' num2str(.1*N)])
...
title(['\Delta log-L vs. additional params.'])
...
hold on
plot(x,chi2cdf(x*log(N),x,'upper'),'LineWidth',2,'DisplayName',['BIC, N=' num2str(N)])
plot(x,chi2cdf(x*log(.1*N),x,'upper'),'LineWidth',2,'DisplayName',['BIC, N=' num2str(.1*N)])
xlabel('Additional number of parameters')
