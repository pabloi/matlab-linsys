...
%% Identify
    ...
    for k=1:2
        ...
        model{D1+1,k}.name=['EM (comm,' num2str(D1) ',' num2str(k) ' in)']; %Robust mode does not do fast filtering
    end
end
%%
save EMrealDimCompare1500comm_2inALT.mat
%% COmpare
vizModels(model(3:5))
%%
%vizDataFit(model([4:-1:1]),Y,U)
%vizDataFit(model([4:-1:1]),Y_p,U_p)
vizDataFit(model(3:5,1),Yf,Uf(1,:))
vizDataFit(model(3:5,2),Yf,Uf)
