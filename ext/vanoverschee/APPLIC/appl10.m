...
%
% Data:
%          The data is contained in the file appl10.mat
%          
% Copyright: 
...
disp('                        of ')
disp('           The thermic resistance of a wall');
disp('          ----------------------------------');
disp(' ');
...
ax_id = [1:1200];n_id = length(ax_id); 	% Identification axis
ax_val = [1201:1680];n_val = length(ax_val); % Validation axis
...
disp('      This may take up to 1 minute')
disp(' ')  
disp('      Hit any key to continue');
disp(' ')
pause
  ...
  tic;
  th_pem = pem([y_id,u_id],thi_pem,[],10);
  th_oe = pem([y_id,u_id],thi_oe,[],10);
  tt2 = toc;
...
show_res('Wall temperature',m,l,n_id,n_val,...
