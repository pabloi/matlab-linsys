...
%
% Data:
%          The data is contained in the file appl6.mat
%          
% Copyright: 
...
ax_id = [1:3300];n_id = length(ax_id); 	% Identification axis
ax_val = [3301:6305];n_val = length(ax_val); % Validation axis
u_id = u(ax_id,:);y_id = y(ax_id,:); 	% Identification data
u_val = u(ax_val,:);y_val = y(ax_val,:); % Validation data
i = 10; 				% Number of block rows
...
disp(' ')
disp('      The following optimization took 8 minutes on a Pentium')  
disp(' ')  
...
show_res('An Evaporator',m,l,n_id,n_val,...
