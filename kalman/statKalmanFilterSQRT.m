function [X,P,Xp,Pp,rejSamples,logL]=statKalmanFilterSQRT(Y,A,C,Q,R,varargin)
...
for i=firstInd:M
  ...
  if ~any(isnan(y)) %If measurement is NaN, skip update.
     [prevX,prevcPt,logL(i),rejSamples(i),icS]=KFupdateSQRT(Ct,cRt,y,prevX,prevcPt,rejThreshold);
  end
  X(:,i)=prevX;  P(:,:,i)=prevcPt; %Store results

  %Then, predict next step:
  [prevX,prevcPt]=KFpredictSQRT(A,cQt,prevX,prevcPt,BU(:,i));
  if nargout>2 %Store Xp, Pp if requested:
      Xp(:,i+1)=prevX;   Pp(:,:,i+1)=prevcPt;
  end
end
