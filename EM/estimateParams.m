...
AB=[sum(Pt,3) xu1]*pinv(O+1e-5*eye(N+1));
A=AB(:,1:size(X,1));
B=AB(:,size(X,1)+1:end);
...
%Q,R:
Q=mean(P(:,:,2:end),3) - A*mean(Pt,3)' -B*xu1'/size(xu1,1); %A*P' is an ill-defined product here I mean the matrix product for each 2D slice along the third dim of Pp 
Q=.5*(Q+Q');
Q=Q+1e-5*eye(size(Q));
R=(Y*Y'-C*X*Y'-D*U*Y')/size(Y,2); %This estimate is weird: does not result in a symmetric matrix
R=.5*(R+R');
R=R+1e-5*eye(size(R));
